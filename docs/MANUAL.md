...
### Import image as shapes

`File->Import Image as Shapes...` (also on the toolbar) converts an SVG or raster image (PNG, JPEG, and the other guide formats) straight into vinyl shapes, skipping the manual guide layer, Bucket, Pen, and grouping steps. Every filled object in an SVG, including strokes and live text, is fitted with shapes the same way a Bucket-derived Pen contour is, and the result is inserted as one group named after the file with one child group per object, centred on the current view at one world unit per SVG pixel.

A raster image is first split into colour regions, one per palette colour, with pixels below the alpha threshold left genuinely empty, so a transparent background stays empty and a light outline effect around a shape becomes its own region. Larger regions are inserted first so smaller ones sit on top. An options dialog appears after choosing the file: **Alpha threshold** (how opaque a pixel must be to count), **Maximum colours** and **Minimum region area** (the two controls that keep anti-aliased edges from becoming hundreds of slivers; lower the colour count or raise the area if an import produces many tiny groups), **Speckle size** and **Trace smoothing** (how the region outlines are traced), **Process at most** (larger images are scaled down before tracing so big regions do not run out of fill time; shapes are still placed at the source size), **Overlap neighbours** (each colour region extends this many pixels underneath the smaller regions drawn on top of it, so adjacent shapes overlap instead of leaving a hairline seam; nothing visible changes size, and 0 traces regions exactly as extracted), and **Separate thin lines** (give thin high-contrast strokes their own regions). **Outline simplification** applies to SVG files too: at the default *Keep source curves* the outlines are used exactly as drawn, while a distance in pixels resamples every outline to within that distance before fitting, which gives fewer shapes at the cost of curve fidelity. The dialog remembers its values. Objects with interior cutouts keep their holes. The import runs in the background; the status bar shows progress and the summary, and the Cancel Active Fill shortcut stops it. Complex artwork is split into connected pieces and each piece is filled on its own, so a single failing piece does not lose the rest. If any piece could not be filled, a warning lists how many, why, and where to find `image_import.log` (next to the executable) with the details; the imported group is still inserted, with those pieces missing. SVGs that use gradients, patterns, masks, clipping, filters, embedded images, text on a path, or a `<style>` block cannot be converted this way yet; import those as a guide layer and use Bucket instead. Objects that cannot be fitted are skipped and counted in the summary. Raster images (PNG, JPEG) are not supported by this action yet.

### 3D preview

With Livery project open or car model loaded in the Group project, you can see the car model preview with the decals applied. You may have to open the widget manually if not present with `Window->3D Preview`. Rotate model with left mouse button, zoom with scrollwheel and pan with middle mouse. With model loaded you can enable UV unwrap and/or wireframe with `Options->Show Car UV unwrap` and `Options->Show Section Wireframe`. The connected preview side panel starts collapsed, has `PAINT`, `PARTS`, and `LOD` tabs, and can be opened or hidden with its arrow button. `PAINT` edits each supported region's primary color, secondary color, and Gloss, Semigloss, Metallic, or Matte finish. Its Body option applies color and finish changes to the hood, mirrors, and wing while `Override hood, mirrors and wing` is checked. `Clear all` removes every paint override and can be undone. Color and finish edits are saved in the project and exported livery. `PARTS` selects body kits, rear wings/spoilers, front and rear bumpers, hoods, and side skirts; compound body-kit selections switch their compatible models together. `LOD` selects the rendered model detail level. The LOD cycle command can also be assigned in `Settings->Keybinds`.
