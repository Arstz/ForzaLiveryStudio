...
`File->Import Image as Shapes...` (also on the toolbar) converts an SVG file straight into vinyl shapes, skipping the manual guide layer, Bucket, Pen, and grouping steps. Every filled object in the SVG, including strokes and live text, is fitted with shapes the same way a Bucket-derived Pen contour is, and the result is inserted as one group named after the file with one child group per object, centred on the current view at one world unit per SVG pixel. Objects with interior cutouts keep their holes. The import runs in the background; the status bar shows progress and the summary, and the Cancel Active Fill shortcut stops it. Complex artwork is split into connected pieces and each piece is filled on its own, so a single failing piece does not lose the rest. If any piece could not be filled, a warning lists how many, why, and where to find `image_import.log` (next to the executable) with the details; the imported group is still inserted, with those pieces missing. SVGs that use gradients, patterns, masks, clipping, filters, embedded images, text on a path, or a `<style>` block cannot be converted this way yet; import those as a guide layer and use Bucket instead. Objects that cannot be fitted are skipped and counted in the summary. Raster images (PNG, JPEG) are not supported by this action yet.
