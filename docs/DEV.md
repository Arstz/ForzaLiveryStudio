...
- Edit layers with Select, Move, Marquee, Transform, Rotate, Pipette, Pen, and
  ...
  Polygonal Lasso validates a simple straight-edged contour and ear-clips an exact
  Triangle baseline. A deterministic shared-edge pass first combines every pair
  whose exact union is a Rectangle or skewed Rectangle (parallelogram). It then
  derives additional tight rotated Rectangle candidates from bounded pairs of
  current triangles, rejects every candidate whose boundary leaves the lasso,
  and greedily replaces two or more still-needed triangles per Rectangle.
  Each accepted merge strictly lowers the shape count. Up to three interior
  anchors are tried as centre fans or local Triangle splits, but only when
  Rectangle savings keep the result at or below the baseline count. Rectangle
  optimization operates only on the interior mesh and never moves or snaps the
  input contour.
...
- `src/gui/canvas/` (`project_canvas.*` plus its per-concern implementation units
  ...
  - OpenGL canvas, pan/zoom, coordinate rulers, project guidelines, hit testing, guide layer rendering, selection
    ...
    deterministic ear clipping, contained rotated-Rectangle merging and optional
    count-funded interior-anchor splits for Polygonal Lasso, and affine primitive
    placement;
