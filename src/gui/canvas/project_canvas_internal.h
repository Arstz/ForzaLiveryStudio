...
inline QTransform flatEntryTransform(const fls::scene::Shape &layer) {
...
inline QTransform flatEntryTransform(const fls::scene::GuideLayer &guide) {
    QTransform transform;
...
inline QSizeF flatEntrySize(const fls::scene::Shape &layer, const QSizeF &vectorSize) {
    return layer.raster ? QSizeF(layer.rasterWidth, layer.rasterHeight) : vectorSize;
}

inline QRectF flatEntryRect(const fls::scene::Shape &layer, const QSizeF &vectorSize) {
    return sceneLocalRect(flatEntrySize(layer, vectorSize));
}

inline QRectF flatEntryVisualRect(const fls::scene::Shape &layer, const ShapeGeometryStore &geometry) {
    if (layer.raster) {
...
inline void collectGuideIds(const fls::scene::Layer &node, QSet<QString> &out) {
