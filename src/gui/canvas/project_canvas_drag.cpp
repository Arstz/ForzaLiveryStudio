...
bool transformsEqual(const fls::scene::Transform2D &left,
                     const fls::scene::Transform2D &right) {
...
void ProjectCanvas::cycleFlipSelection() {
    ...
    if (continueCycle) {
        for (const QString &id : targetIds) {
            const fls::scene::Layer *node = state_->sceneNode(id);
    ...
    if (!continueCycle) {
        ...
        for (const QString &id : targetIds) {
            if (const fls::scene::Layer *node = state_->sceneNode(id)) {
    ...
    for (const QString &id : targetIds) {
        if (fls::scene::Layer *node = state_->sceneNode(id)) {
    ...
    const auto applyFlip = [&](auto *item) {
        ...
        const fls::scene::Transform2D baseline = flipCycle_->baseline.value(item->id);
        const double skew = [&] {
            if constexpr (std::is_same_v<Item, fls::scene::Shape>) {
