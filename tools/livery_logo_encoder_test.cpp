...
bool testLogoEncoding() {
    ...
    QByteArray expectedTopRemnant(1, '\0');
    fls::detail::appendLeU16(expectedTopRemnant, static_cast<quint16>(0x8000u | 129u));
    expectedTopRemnant.append(QByteArray(6, '\0'));
    fls::detail::appendLeFloat(expectedTopRemnant, 1.0f);
    fls::detail::appendLeFloat(expectedTopRemnant, 0.0f);
    expectedTopRemnant.append('\x80');
    if (!body.contains(expectedTopRemnant)) {
        qCritical() << "grouped terminal metadata did not retain the logo identifier";
        return false;
    }
...
int main(int argc, char *argv[]) {
    ...
    return testLogoEncoding() && testLogoShapeLimit() && testPartialSourceRebuild() ? 0 : 1;
}
